...
2. Push-to-talk in `NitidoChatPage` → full tool registry → conversational answers grounded in live DB data.
...
- Two `AgentProfile`s: `quickExpense` (single forced tool), `nitidoAssistant` (`tool_choice: auto`, max 3 loops).
...
| `lib/core/services/ai/agent_profiles.dart` | New | `quickExpense`, `nitidoAssistant`. |
...
| `lib/app/chat/nitido_chat.page.dart` | Modified | Mic button, `_sendToAgent`, tool-call approval bubble. |
...
3. Revert `nitido_chat.page.dart` to pre-refactor `_send` (git revert of that commit).
...
- **`nitido-ai-integration`** — landed on main. Provides `NexusAiService`, `NexusCredentialsStore`, `FinancialContextBuilder`, `SettingKey.nexusAiEnabled` + sub-toggles, `NitidoChatPage`, `AiSettingsPage`. This change extends them non-breakingly.
...
4. **Tanda 4 — Chat surface** (independent of Tanda 5): mic button in `NitidoChatPage`, `_sendToAgent` refactor, approval UI for mutating tools, streaming preserved for no-tool responses.
...
- [ ] In `NitidoChatPage`, user taps mic and asks *"¿cuánto gasté en comida en marzo?"* → agent runs `get_stats_by_category` tool → replies with a grounded text answer in Spanish; streaming works for the final text.
