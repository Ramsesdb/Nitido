...
- `lib/i18n/generated/translations_en.g.dart` contains 74 `bolsio_ai` references.
- `lib/i18n/json/es.json` line 477 opens the `BOLSIO-AI` group; critical keys present:
...
| Scenario: Noisy input yields editable text | met | Empty string propagates to caller; `voice_capture_flow.dart:46-51` + `bolsio_chat.page.dart:92-97` handle the empty-transcript edge case. |

### ai-tools/spec.md

| Requirement / Scenario | Status | Evidence |
|---|---|---|
| **Tool Registry and Schema Serialization** — 6 tools, OpenAI shape | met | `ai_tool_registry.dart:40-51` (`toOpenAiTools`). `bolsio_ai_agent.dart:39-46` registers all 6 tools. |
...
| Scenario: Cap reached | met | `bolsio_chat.page.dart:161-162` renders `t.chat_error_loop_cap` (= "No pude completar la consulta"). |
| **Mutating Tool Approval Gate** | met | `agent_runner.dart:97-123` — builds `PendingApproval` list and short-circuits with `AgentRunStatus.needsApproval` when `profile.requiresApproval(name)` is true. `bolsio_ai_agent.dart:55` declares `{create_transaction, create_transfer}` as `approvalRequiredTools`. |
| Scenario: Approval required for create_transaction | met | `bolsio_chat.page.dart:211-236` opens `showToolApprovalSheet` BEFORE `dispatch(...)` is called. |
| Scenario: User rejects mutation | met | `bolsio_chat.page.dart:238-245` appends `{"error":"user_rejected"}` when `approved != true`. |
| **Agent Profile Isolation** — `quickExpense` vs `bolsioAssistant` | met | `quick_expense_agent.dart:33-48` (forced tool, maxLoops=1, propose registry) vs `bolsio_ai_agent.dart:39-56` (all 6, `auto`, maxLoops=3). |
...
| Requirement / Scenario | Status | Evidence |
|---|---|---|
| **Voice Input Affordance in Chat** — mic gated on both settings | met | `bolsio_chat.page.dart:322-326,477-495` — `voiceAffordance = nexusAiEnabled && aiVoiceEnabled`; mic button only rendered when true. |
| Scenario: Voice-driven question | met | `bolsio_chat.page.dart:74-101` — `_onMicTap` → permission → `showVoiceRecordOverlay` → `_sendToAgent`. Agent has `get_stats_by_category` (`bolsio_ai_agent.dart:41`). |
| Scenario: Voice disabled hides mic | met | `bolsio_chat.page.dart:477` (`if (voiceAffordance) ...`) with `aiVoiceEnabled != '0'` check at line 325. |
| **Tool-Enabled Message Flow** — `_sendToAgent`, `completeWithTools`, streaming fallback | met | `bolsio_chat.page.dart:109-143` (`_sendToAgent`) + `agent_runner.dart:58-64` + `bolsio_chat.page.dart:150-152` (`streamFinalText` → `_streamFinalText(messages)`). |
| Scenario: Plain question streams as before | met | `agent_runner.dart:58-64` returns `AgentRunStatus.streamFinalText` on first-iter no-tools; `bolsio_chat.page.dart:174-203` runs `NexusAiService.streamComplete(...)` with the same messages. |
| Scenario: Tool call runs non-streaming, then final text streams | **partial** (warning) | Non-streaming tool loop works. Post-tool summary renders via `bolsio_chat.page.dart:153-158` directly from `result.finalText`, NOT re-streamed via `streamComplete`. Deviation documented in `apply-progress.md` Tanda 4 §"Streaming path narrowed". Behaviour is functional; spec's "streams" phrasing is the miss. |
| **Approval Bubble for Mutating Tools** | met | `bolsio_chat.page.dart:220-224` opens approval sheet with decoded arguments; `bolsio_chat.page.dart:688-910` (`_ToolApprovalSheet`) renders summary rows per tool. |
| Scenario: User approves a chat-proposed expense | met | `bolsio_chat.page.dart:227-237` dispatches through `_agent.profile.toolRegistry.dispatch(...)` after approve, appends JSON-encoded result as `role:'tool'`, then `_agent.resume(...)`. |
| **Tool-Loop Cap Surfacing in Chat** — fallback message | met | `bolsio_chat.page.dart:161-162` renders `t.chat_error_loop_cap` ("No pude completar la consulta."), and the input row re-enables via `setState(_isSending = false)` in the `finally` block at line 134-142. |
...
| **Empty Transcript Handling** — no agent call, toast | met | `voice_capture_flow.dart:46-51` shows `BolsioSnackbar.warning(t.voice_empty_transcript)` and short-circuits BEFORE any agent call. |
...
| Scenario: Default value on fresh install with AI enabled | met | Code-level default via `!= '0'` pattern: `new_transaction_fl_button.dart:200-201`, `bolsio_chat.page.dart:324-325`, `ai_settings.page.dart:48,73`. Null → ON. |
...
| **AI Settings Page Toggle** | met | `ai_settings.page.dart:228-235` `SwitchListTile` bound to `_aiEnabled && _voiceEnabled`, `onChanged` disabled when `!_aiEnabled`. Uses `t.bolsio_ai.voice_settings_title` / `voice_settings_subtitle` (note: design said `t.settings.ai.voice_input.title`; consolidated under `BOLSIO-AI` group per Tanda 6 deviation — spec-compatible because the title/subtitle distinction is preserved). |
...
| Module layout: `lib/core/services/ai/agents/` | yes | 5 files: `agent_profile.dart`, `agent_run_result.dart`, `agent_runner.dart`, `quick_expense_agent.dart`, `bolsio_ai_agent.dart`. (Design sketched 3 files; runner + result type were split out as documented deviation — design-compatible refactor.) |
...
| Dual gate `nexusAiEnabled && aiVoiceEnabled` | yes | `new_transaction_fl_button.dart:198-202`, `bolsio_chat.page.dart:322-326`, `ai_settings.page.dart:228-232`. All use the `!= '0'` form for default-ON behavior. |
...
| Fix | Status | Evidence |
|---|---|---|
| 1. `appStateSettings` hoisted to locals at top of `build()` in `bolsio_chat.page.dart` | yes | `bolsio_chat.page.dart:319-327` — 4 hoisted locals (`nexusAiEnabled`, `aiVoiceEnabled`, `voiceAffordance`, `avatarId`) before any conditional or widget tree. Boot-state branch early-returns its own Scaffold at `:329-346`. |
| 2. Dedicated `FocusNode` on TextField, initialized + disposed | yes | `bolsio_chat.page.dart:28` declares `final _inputFocus = FocusNode();`. `:45` disposes it. `:453` wires it via `focusNode: _inputFocus`. No listeners attached (intentional per Tanda 6 notes). |
| 3. `RepaintBoundary` wrapping assistant `MarkdownBody` | yes | `bolsio_chat.page.dart:417-423` — `RepaintBoundary(child: MarkdownBody(...))` only on non-user, non-thinking branch. User bubbles and typing indicator remain unwrapped (intentional — cheap widgets). |
...
| Tanda 4: Temperature lowered to 0.3 on streaming path | yes | Matches `BolsioAiAgent` default; plain chat still conversational. |
...
| Tanda 6: Single `BOLSIO-AI` i18n group (not `transaction.voice_input` + `chat.voice`) | yes | All required surfaces have keys under the consolidated group. The `voice_settings_*` prefix preserves the title/subtitle distinction called for in settings §AI Settings Page Toggle. |
