...
Introduce voice input to Nitido in two surfaces that share one STT + tool-calling stack:
...
2. **Conversational voice in the Nitido AI chat** — push-to-talk next to the send button, full tool registry (`get_balance`, `list_transactions`, `get_stats_by_category`, `get_budgets`, `create_transaction`, `create_transfer`, etc.), open questions in Spanish.

Shared plumbing: one `VoiceService` (STT abstraction), one `AIToolRegistry` with Dart-callable tools, two agent profiles (`quick-expense-agent`, `nitido-ai-agent`) that reuse the existing `NexusAiService`. Full realtime streaming ("live mode" / Gemini Live / OpenAI Realtime) is explicitly out of scope for this change.
...
- `lib/core/services/ai/nexus_ai_service.dart` — OpenAI-compatible client against `https://api.ramsesdb.tech/v1/chat/completions`. Three methods today:
  ...
  - `streamComplete(messages, temperature)` — SSE stream, yields `String` deltas, used by `NitidoChatPage`.
...
- `lib/core/services/ai/financial_context_builder.dart` — bakes accounts + 60 last tx + categories + budgets into a ~8k-char system-prompt blob. Used by `NitidoChatPage._bootstrap`. This is the **pre-computed context** pattern; switching to tools is an alternative architecture, not an addition.

### Chat surface

- `lib/app/chat/nitido_chat.page.dart` — self-contained `StatefulWidget` with in-memory `_messages` list, `TextEditingController`, manual SSE aggregation in `_send()`. Input row is an inline `Row` with `TextField` + `IconButton.filled` (send). Avatar, markdown rendering, typing indicator all present. No audio/mic code anywhere.
- `lib/app/ai/ai_hub.page.dart` — entry-point hub with a card linking to `NitidoChatPage` plus insights.
...
- `lib/core/services/ai/agent_profiles.dart` — `AgentProfile { name, systemPrompt, tools, temperature, maxToolLoops }`:
  ...
  - `nitidoAssistant` → system = full financial-assistant prompt, tools = all, `tool_choice: 'auto'`, max 3 loops.
...
- **Chat surface** — `lib/app/chat/nitido_chat.page.dart`:
...
| `lib/app/chat/nitido_chat.page.dart:316` | Insert `_MicButton(onResult: (text) => _sendWithVoice(text))` before the `TextField` in the input Row | Reuses `_send` pathway with `text` prefilled. |
| `lib/app/chat/nitido_chat.page.dart:_send` | Refactor to `_sendToAgent(profile: AgentProfile.nitidoAssistant, userText: text)` | Agent loop handles tool calls transparently. |
...
| `lib/core/services/ai/agent_profiles.dart` (new) | 2 profiles | `quickExpense`, `nitidoAssistant`. |
...
10. **Agent loop runaway** — Model could keep calling tools. Mitigate: hard-cap `maxToolLoops = 3` in `nitidoAssistant` (scoped agent allows only 1). Log every loop iteration with telemetry.
11. **Conflict with in-progress `NitidoChatPage`** — the current page's `_send` handles streaming directly. Refactoring to agent-loop means the streaming UX for plain conversational replies (no tools) must be preserved. Mitigate: agent loop detects "no tool_calls" on first response → switches to `streamComplete` path for the final text.
...
### `nitido-ai-integration` — FOUNDATIONAL DEPENDENCY (already landed per current code)
...
Only add mic to `NitidoChatPage`; users dictate "créame un gasto de 45 en uber" through the chat agent.
...
>
> No conflict with `attachments-and-receipt-ocr` (parallel sibling, both add a `CaptureChannel` case on `TransactionProposal`). Depends on `nitido-ai-integration` (already landed). Ready to run `/sdd-propose`.
