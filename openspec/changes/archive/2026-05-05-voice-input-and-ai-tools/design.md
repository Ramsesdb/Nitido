...
| Scoped tool exec | `CreateTransactionTool` emits a `TransactionProposal` (no DB write) in `quickExpense`; writes in `nitidoAssistant` after approval | Two separate tool classes | Single tool shape, `execMode` flag toggles commit; less surface. |
...
| Agent loop cap | `maxToolLoops = 3` for `nitidoAssistant`, `1` for `quickExpense` | Unbounded | Prevents runaway cost/latency; 3 covers nearly all realistic multi-tool questions. |
...
Chat mic → VoiceService → text → nitidoChatPage._sendToAgent
                                              │
                  ┌───────────────────────────┘
                  ▼
...
| `lib/core/services/ai/agents/nitido_ai_agent.dart` | Create | Profile + `run(messages) → Stream<AgentEvent>` runner with tool/approval/final-text events. |
...
| `lib/app/chat/nitido_chat.page.dart` | Modify | Mic button, refactor `_send` → `_sendToAgent`, approval bubble widget, stream fallback when no tool_calls. |
...
class AgentProfile {
  ...
  bool requiresApproval(String toolName) => ...; // nitidoAssistant: true for mutating
}
...
for i in 0..profile.maxToolLoops:
  ...
  if r.toolCalls.isEmpty:
    if i == 0 and profile == nitidoAssistant:
