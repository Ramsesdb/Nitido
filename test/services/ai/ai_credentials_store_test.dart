...
void main() {
  ...
  group('AiCredentialsStore — basic CRUD', () {
    ...
    test('loadCredentials returns null for unset provider', () async {
      expect(await store.loadCredentials(AiProviderType.gemini), isNull);
    });
    ...
    test('deleteCredentials removes the entry', () async {
      ...
      expect(await store.loadCredentials(AiProviderType.openai), isNull);
    });
    ...
    test(
      'saveCredentials normalises whitespace and zero-width chars',
      () async {
        await store.saveCredentials(
          const AiCredentials(
            providerType: AiProviderType.openai,
  ...
  group('AiCredentialsStore — legacy migration', () {
    test(
      'migrateFromLegacyStore moves legacy Nexus key to new store',
      () async {
        FlutterSecureStorage.setMockInitialValues({
    ...
    test(
      'migrateFromLegacyStore is idempotent — running twice is safe',
      () async {
        FlutterSecureStorage.setMockInitialValues({
          'nexus_ai_api_key': 'legacy-sk',
        });
    ...
    test(
      'migrateFromLegacyStore does not overwrite an existing new entry',
      () async {
        FlutterSecureStorage.setMockInitialValues({
          'nexus_ai_api_key': 'legacy-sk',
        });
