...
void main() {
  ...
  group('Onboarding persistence shape per mode (Task 10.11)', () {
    test('single_usd → currencyMode=single_usd, preferredCurrency=USD, '
      ...
      expect(
        ...
        isFalse,
        reason:
            's03 not shown for single mode → rateSource MUST NOT '
            'be written.',
      );
    });
    ...
    test('single_bs → currencyMode=single_bs, preferredCurrency=VES', () {
      ...
      expect(writes[SettingKey.secondaryCurrency], isNull);
      expect(writes.containsKey(SettingKey.preferredRateSource), isFalse);
    });
    ...
    test('dual(USD, VES) → primary=USD, secondary=VES, rateSource WRITTEN '
        '(s03 shown)', () {
      final writes = applyChoicesWriteSet(
      ...
      expect(
        ...
        'paralelo',
        reason:
            's03 was shown for dual(USD,VES) — the user pick MUST '
            'be persisted.',
      );
    });

    test('dual(VES, USD) inverted order → primary=VES, secondary=USD, chip '
    ...
    test('dual(EUR, ARS) → rateSource NOT written (s03 not shown for non '
        'USD↔VES dual)', () {
      final writes = applyChoicesWriteSet(
        selectedMode: CurrencyMode.dual,
        selectedPrimaryCurrency: 'EUR',
    ...
    test('dual with null secondary defaults to VES (legacy fallback)', () {
      final writes = applyChoicesWriteSet(
        selectedMode: CurrencyMode.dual,
        selectedPrimaryCurrency: 'USD',
        selectedSecondaryCurrency: null,
  ...
  group('Onboarding output round-trips via the resolver', () {
