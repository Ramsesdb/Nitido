...
void main() {
  ...
  group('CaptureOrchestrator', () {
    ...
    test(
      'event with sender 2661 but non-parseable text (OTP) produces no pending_import',
      () async {
        await _insertAccount(db, id: 'acc-bdv', name: 'Banco de Venezuela');
    ...
    test(
      'valid SMS event produces exactly 1 pending_import with status=pending',
      () async {
        await _insertAccount(db, id: 'acc-bdv', name: 'Banco de Venezuela');
    ...
    test(
      'duplicate event (same bankRef) is skipped and does not create a new pending_import',
      () async {
        await _insertAccount(db, id: 'acc-bdv', name: 'Banco de Venezuela');
  ...
  group('CaptureOrchestrator — notification channel (Tanda 3A)', () {
    ...
    setUp(() {
      fakeNotifSource = FakeCaptureSource(channel: CaptureChannel.notification);
    });
      ...
      () async {
        ...
        fakeNotifSource.emit(
          RawCaptureEvent(
            ...
            sender:
                'com.bancodevenezuela.bdvdigital', // Must match BdvNotifProfile.knownSenders
