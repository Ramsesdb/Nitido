...
void main() {
  ...
  group('Dashboard mixed-currency totals — DualMode(USD, VES) BCV↔Paralelo '
      'toggle (Task 10.7)', () {
    ...
    // its own.
    Stream<Map<String, double>> mixedAccountsStream() =>
        Stream.value({'USD': usdNativeBalance, 'VES': vesNativeBalance});

    test('BCV: total = 100 USD (native) + 1000/40 = 125 USD', () async {
    ...
    test('Paralelo: total = 100 USD (native) + 1000/45 ≈ 122.22 USD', () async {
      final helper = CurrencyConversionHelper.forTesting(
    ...
    test('CRITICAL: native USD portion is INVARIANT across BCV ↔ Paralelo '
        'toggle (the absorbed bug)', () async {
      ...
      final expectedParaleloVesPortion = vesNativeBalance / paraleloVesPerUsd;
      final expectedDelta = expectedBcvVesPortion - expectedParaleloVesPortion;
      final actualDelta =
      ...
      expect(
        ...
        reason:
            'BCV total minus VES-converted portion MUST equal the '
      ...
      expect(
        ...
        closeTo(usdNativeBalance, 1e-9),
        reason:
            'Paralelo total minus VES-converted portion MUST equal '
            'the native USD balance verbatim.',
      );
    });

    test('policy.rateSourceForPair routes USD↔VES via the user preference', () {
    ...
    test('RateProviderChain.sourceForPair agrees with policy on USD↔VES', () {
  ...
  group('Fallback chain integration — Frankfurter → manual → null '
      ...
      final helper = CurrencyConversionHelper.forTesting(
        rateLookup:
            ({
              ...
              fail(
                'rateLookup invoked for identity pair $fromCurrency→$toCurrency',
              );
            },
      );
    ...
    test('Missing rate (Frankfurter 404 + no manual override) surfaces in '
        'missingRateCurrencies set', () async {
      final helper = CurrencyConversionHelper.forTesting(
        rateLookup:
            ({
              required String fromCurrency,
  ...
  group('SingleMode end-to-end — VES base with mixed accounts (Task 10.7 '
      'companion)', () {
    test('single_bs with USD account 50 + VES account 500 at BCV=40 → '
        '2500 VES total', () async {
